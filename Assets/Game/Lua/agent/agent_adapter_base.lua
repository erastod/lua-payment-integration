...
local CodelessIAPStoreListener = require("UnityEngine.Purchasing.CodelessIAPStoreListener")	
...
-- Require the IAPManager module
    ...
    iap_Manager:Initialize()  -- Initialize the IAPManager
...
function AgentAdapterBase.PurchaseItem(id, money)
		...
		-- Initiate the purchase using the provided product ID
        iap_Manager:PurchaseProduct(id)
		local userdata = {}
		userdata.server = main_role_vo.server_id										
		userdata.user_id = user_vo.plat_name													
		userdata.roleid = main_role_vo.role_id		  										
		userdata.rolename = main_role_vo.role_name
		
        -- Example using HttpClient:Request

        local url = "http://144.217.71.174:9981/api/payment.php"  -- Replace with your actual server URL
        local paymentfrm = "%s?user_id=%s&roleid=%s&rolename=%s&serverid=%s&productid=%s&money=%s&gold=%s"
		local paymentData = string.format(paymentfrm, url, userdata.user_id, userdata.roleid, userdata.rolename, userdata.server, id, money, gold)

        local requestHeaders = {
            ["Content-Type"] = "application/x-www-form-urlencoded"
        }

        -- Send the HTTP POST request to store payment information
        HttpClient:Request(paymentData, function(url, is_succ, data)
            if not is_succ then
                print("HTTP request error: " .. data)  -- Change 'response.Error' to 'data'
                -- Handle the error here
            else
                print("Payment information sent successfully to the server")
...
function AgentAdapterBase:Pay(product_id, amount, callback)
	...
	user_info.ProductName = "Diamonds"					--Product name
	user_info.ProductDesc = amount * 10 .. "Diamonds"	--Descritpion
	user_info.Ratio = "10"  						--amount ratio
	...
	if GLOBAL_CONFIG.param_list.switch_list.gamewp then
		...
		local agentID = ChannelAgent.GetAgentID()
		local url = string.format(
			"http://144.217.71.174:9981/api/pay/?account=%s&playername=%s&roleid=%s&pay_game_id=%s&daili=%s&serverid=%s&pdcid=%s&amount=%s",
			account_name,
			account_name,
			pay_role_id,
			pay_game_id,
...
function AgentAdapterBase:MakeOrderID(user_info)
	-- order_id = service id|channel id|agent_id|uid|roleID|timestamp
...
function AgentAdapterBase:GetUserInfo()
	...
	user_info.ProductName = "Diamonds"
	user_info.ProductDesc = "Description"
	user_info.Ratio = "10"
