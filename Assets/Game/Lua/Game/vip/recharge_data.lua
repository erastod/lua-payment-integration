...
-- Mainui_view Exclusive for recharge red dot
...
function RechargeData:GetRechargeIdList()
	if self.recharge_data_list == nil then
		...
		elseif self.platform == PHONE_TYPE.IPHONE then
			...
			-- Force the use of 1 Android configuration (for ios jailbreak)
			-- Channel IDs starting with the letter i are all genuine iOS (as Jiannan said)
...
--Get a column of information corresponding to the recharge ID
...
--Obtain the collection that needs to load the index through the index
