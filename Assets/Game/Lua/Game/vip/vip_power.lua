...
VipPowerId =
{
	...
	-- act_qibing 						= 22,			--激活骑兵
	fuben_phase_buy_times			= 22,			--进阶副本进入次数
	...
	materials_fb_buy_times			= 35,			--武器材料副本进入次数
	build_tower_fb_buy_times		= 36,			--建塔副本进入次数
	armor_fb_buy_times				= 38,			--防具材料副本进入次数
}
...
--获得当前vip权限下的参数
function VipPower:GetParam(power_id, is_next)
	...
	if is_next then
		vip_level = PlayerData.Instance.role_vo.vip_level + 1
	end
...
function VipPower:GetNextVipLevelLimint(power_id, vip_level)
	local vip_cfg = self:GetPowerCfg(power_id)
	if nil == vip_cfg and nil == vip_level then
		return 0
	end
	local max_vip_num = VipData.Instance:GetVipMaxLevel() or 0
	for i = 1, max_vip_num do
		if i > vip_level and vip_cfg["param_" .. i] > vip_cfg["param_" .. vip_level] then
			return i, vip_cfg["param_" .. i]
		end
	end
	return -1, -1
end