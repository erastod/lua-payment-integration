...
function VipCtrl:OnVipInfo(protocol)
	...
	kaifu_view:FlushLevelInvestmentView()
	kaifu_view:FlushMonthCardInvestmentView()
	...
	if scene_obj ~= nil and scene_obj:IsMainRole() then
		...
		MainUICtrl.Instance:FlushView("temp_vip")
		self:CheckTempVipView()
		self.vip_view:Flush()
		self.vip_view:FlushRewardState()
		RemindManager.Instance:Fire(RemindName.RechargeAndVIP)
