...
function RechargeItem:OnFlush()
	...
	self.node_list["Txt"].text.text = string.format("Usd%s", " "..recharge_cfg.money)
...
function RechargeItem:OnRechargeClick()
    ...
    if not recharge_cfg then
     ...
     -- Add the product to IAPManager with ID and price
	local Consumable = UnityEngine.Purchasing.ProductType.Consumable
    iap_Manager:AddProduct(recharge_cfg.id, Consumable)
...
function RechargeItem:SendRecharge(recharge_cfg)
    RechargeCtrl.Instance:Recharge(recharge_cfg.id, recharge_cfg.money)
end
