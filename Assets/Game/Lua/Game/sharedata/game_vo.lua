...
-- 塔防副本防御塔
DefenseVo = DefenseVo or BaseClass(BaseVo)
...
-- 小鬼
ImpGuardObjVo = ImpGuard or BaseClass(BaseVo)
function ImpGuardObjVo:__init()
	self.owner_role_id = 0
	self.owner_obj_id = -1
	self.pos_x = 0
	self.pos_y = 0
...
--结婚巡游
MarryObjVo = MarryObjVo or BaseClass(MarryObjVo)
...
-- 飞宠
FlyPetObjVo = FlyPetObjVo or BaseClass(BaseVo)
function FlyPetObjVo:__init()
	self.owner_role_id = 0
	self.owner_obj_id = -1
	self.pos_x = 0
	self.pos_y = 0
...
function RoleVo:__init()
	self.merge_server_id = 0						-- 合服后服ID
	self.plat_type = 0								-- 平台ID
	self.plat_role_id = 0							-- 平台角色ID
	self.role_id = 0								-- 角色ID
	self.dir = 0									-- 方向
	...
	self.user_pet_special_img = 0					-- 精灵特殊形象
	...
	self.combine_server_equip_active_special =0  	-- 屠龙-传世装备是否激活特效,0不是1是
	self.task_appearn_param_1 = 0					-- 接受任务特殊外观参数
	self.task_appearn = 0							-- 接受任务特殊外观
	self.lingzhu_used_imageid = -1					-- 灵珠
	self.is_yinshen = 0								-- 是否隐身
...
function MainRoleVo:__init()
	...
	self.base_per_baoji_hurt = 0 					-- 暴击伤害万分比
	self.base_per_kangbao_hurt = 0					-- 暴击伤害抵抗万分比
	self.base_zhufuyiji_per = 0						-- 祝福一击率
	self.base_gedang_per = 0						-- 格挡率
	self.base_gedang_dikang_per = 0					-- 格挡抵抗率
	self.base_gedang_jianshang = 0					-- 格挡减伤
	self.base_skill_zengshang = 0					-- 技能增伤
	self.base_skill_jianshang = 0					-- 技能减伤
	self.base_mingzhong_per = 0						-- 基础命中率
	self.base_shanbi_per = 0						-- 基础闪避率
	...
	self.pvp_zengshang = 0 							-- pvp增伤
	self.pvp_jianshang = 0 							-- pvp减伤
	self.pve_zengshang = 0 							-- pve增伤
	self.pve_jianshang = 0 							-- pve减伤
	self.huixinyiji = 0 							-- 会心一击
	self.huixinyiji_hurt_per = 0 					-- 会心一击伤害率
